...
{
    regex='^\s*'"$1"':\s+\S+\s+\<\S+\>'
    foundline=$(git log -n 1 HEAD | egrep -m 1 "$regex")
    if [ -z "$foundline" ]; then
...
{
    regex='^\s*'"$1"':\s+\K(\S+)'
    foundline=$(git log -n 1 HEAD | grep -Po "$regex")
    if [ -z "$foundline" ]; then
        echo "error: missing \"$1\""
        return 1
    fi
...
function new_change_commit()
{
    ...
    long_subject=$(git log -n 1 --pretty=%s HEAD | egrep -m 1 '.{51}')
    ...
    body=$(git log -n 1 --pretty=%b HEAD | egrep -m 1 '.{71}')
    if [ -n "$body" ]; then
...
function is_openzfs_port()
{
    ...
    subject=$(git log -n 1 --pretty=%s HEAD | egrep -m 1 '^OpenZFS')
    if [ -n "$subject" ]; then
        return 0
    fi
...
function openzfs_port_commit()
{
    ...
    subject=$(git log -n 1 --pretty=%s HEAD | egrep -m 1 '^OpenZFS [[:digit:]]+ - ')
    if [ -z "$subject" ]; then
