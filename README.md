...
zfs-buildbot requires buildbot 3.2.0 or later.  buildbot 3.2.0 contains a patch
that prevents zombie instances from getting spawned:

https://github.com/buildbot/buildbot/pull/5401
...
sudo apt-get -y install python3-sphinx python3-pip
sudo apt-get install build-essential nginx
```
### Install dependencies (Fedora)
```
sudo dnf install buildbot nginx python3-treq patch
```
...
```
### Create virtual environment to pip install into (Ubuntu only)
```
python3 -m venv ~/sandbox
```

### Patch buildbot (Ubuntu)
```
patch -d ~/sandbox/lib/python3.6/site-packages --dry-run -p2 ~/zfs-buildbot/master/patches/*.patch
```

### Patch buildbot (Fedora)
```
sudo patch -d /usr/lib/python3.9/site-packages -p2 < ~/zfs-buildbot/master/patches/*.patch
```
...
As root, create `/etc/nginx/nginx.conf` for buildbot:
```
## Un-comment the line below on Ubuntu only:
# user www-data;
...
```
### Enable nginx at boot, and start it up manually
```
sudo systemctl enable nginx
sudo systemctl start nginx
```

### Tweak SELinux rules for web server (Fedora only)
```
sudo setsebool -P httpd_can_network_connect 1
```

### Logout and re-login as `buildbot` user

### Enter virtual environment and install buildbot (Ubuntu only)
```
...
```
### Copy over your `password.py` to `zfs-buildbot/master/password.py`

### Do a one-time update of its buildbot's database
```
